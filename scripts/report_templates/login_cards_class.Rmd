...
#roster <- read_csv(paste0("/Volumes/GoogleDrive/My\ Drive/xSEL\ Labs/Rosters/2019-20/FormingLives/Stilwell/MiddleSchool/", params$school))

roster <- filter(roster_full, class ==params$class) # 
#logo: "xSEL Favicon Final.png"
...
person <- purrr::pmap(roster, function(first_name, last_name, class, student_id, school,
                                       district,...) {
  list(
    name = paste0(first_name, " ", last_name),
    title = paste0("Username & Password: ", params$prefix, ".", student_id),
    email = paste0("Class: ", class),
    phone = paste0("School: ", word(school, 1,str_count(school, boundary("word"))[1])),
